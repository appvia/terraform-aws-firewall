...
| <a name="input_firewall_rules"></a> [firewall\_rules](#input\_firewall\_rules) | A collection of firewall rules to add to the policy | `map(string)` | n/a | yes |
...
| <a name="input_vpc_id"></a> [vpc\_id](#input\_vpc\_id) | If reusing an existing VPC, provide the VPC ID and private subnets ids | `string` | `null` | no |
